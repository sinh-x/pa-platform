...
test("accepts the Pi version boundary only once", async () => {
  ...
  const adapter = new PiAdapter({ cwd: dir, versionProbe: () => { probes++; return "0.80.8"; }, sessionIdFactory: () => "00000000-0000-0000-0000-000000000001", runCommand: (args) => { assert.deepEqual(args.slice(0, 4), ["--print", "--json", "--session-id", "00000000-0000-0000-0000-000000000001"]); return { status: 0, stdout: '{"type":"message","text":"ok"}\n', stderr: "" }; } });
  await adapter.spawn({ primerPath: primer, deployId: "d-aaaaaa", mode: "foreground" }); await adapter.spawn({ primerPath: primer, deployId: "d-bbbbbb", mode: "foreground" }); assert.equal(probes, 1);
});
