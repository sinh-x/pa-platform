...
test("generatePrimer requirements analyze fixture preserves required opencode-safe procedures", () => {
  ...
  assert.match(primer, /For semantic briefing requests \(for example: get up to date\), render a reflection-first related bundle with evidence links, then ask exactly one confirmation question before deeper analysis\./);
  assert.match(primer, /Before confirmation, do not mutate ticket, doc, status, branch, registry, or doc-ref state/);
